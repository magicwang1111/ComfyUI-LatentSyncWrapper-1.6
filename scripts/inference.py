...
def main(config, args):
    ...
    if hasattr(args, 'extension_dir') and args.extension_dir:
    ...
    whisper_model_path = getattr(args, 'whisper_ckpt_path', None)
    if not whisper_model_path:
        whisper_model_path = os.path.join(base_dir, "checkpoints", "whisper", f"{whisper_model_name}.pt")
    if not os.path.exists(whisper_model_path):
        raise RuntimeError(f"Whisper checkpoint '{whisper_model_path}' not found")
    print(f"Loaded Whisper checkpoint path: {whisper_model_path}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--whisper_ckpt_path", type=str, default=None)
    parser.add_argument("--extension_dir", type=str, default=None)
