...
def save_wav_with_soundfile(audio_path, waveform, sample_rate):
    waveform_cpu = waveform.detach().cpu().clamp(-1.0, 1.0)
    if waveform_cpu.dim() == 1:
        audio_data = waveform_cpu.numpy()
    elif waveform_cpu.dim() == 2:
        audio_data = waveform_cpu.transpose(0, 1).numpy()
    else:
        raise ValueError(f"Expected waveform with 1 or 2 dimensions, got shape {tuple(waveform_cpu.shape)}")

    sf.write(audio_path, audio_data, sample_rate, subtype="PCM_16")


def write_video_with_pyav(video_path, frames, fps):
    import av

    frames = frames.detach().cpu()
    if frames.dim() != 4:
        raise ValueError(f"Expected video frames with 4 dimensions, got shape {tuple(frames.shape)}")
...
def setup_models():
    ...
    # Keep every model/cache artifact under the extension checkpoints folder.
    hf_cache_dir = os.path.join(ckpt_dir, ".cache", "huggingface")
    os.makedirs(hf_cache_dir, exist_ok=True)

    unet_path = os.path.join(ckpt_dir, "latentsync_unet.pt")
    whisper_path = os.path.join(whisper_dir, "tiny.pt")

    # Only download if models aren't in the extension checkpoint directory.
...
class LatentSyncNode:
    ...
    def inference(self, images, audio, seed, lips_expression=1.5, inference_steps=20):
        ...
        try:
            ...
            save_wav_with_soundfile(audio_path, waveform, sample_rate)

            # Move frames to CPU for saving to video
            frames_cpu = frames.cpu()
            try:
                import torchvision.io as io
                write_video = getattr(io, "write_video", None)
                if write_video is None:
                    raise AttributeError("torchvision.io.write_video is unavailable")
                write_video(temp_video_path, frames_cpu, fps=25, video_codec='h264')
                ...
                if "macro_block_size" in str(e):
                    ...
                    write_video_with_pyav(temp_video_path, frames_cpu, fps=25)
            ...
            read_video = getattr(io, "read_video", None)
            if read_video is None:
                processed_frames = read_video_with_pyav(output_video_path)
            else:
                processed_frames = read_video(output_video_path, pts_unit='sec')[0]
